...
from backend.gamification.models import (
...
class PracticeModelsIntegrationTest(TestCase):
    ...
    def test_unlimited_attempts_workflow(self):
        ...
        for submission in all_submissions[:-1]:
            self.assertEqual(submission.status, 'fail')
            self.assertEqual(submission.points_earned, 0)
        
        self.assertEqual(all_submissions.last().status, 'success')
        self.assertEqual(all_submissions.last().points_earned, 200)
